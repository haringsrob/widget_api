...
class WidgetPluginManager extends DefaultPluginManager implements WidgetPluginManagerInterface {
  ...
  public function __construct(\Traversable $namespaces, CacheBackendInterface $cache_backend, ModuleHandlerInterface $module_handler, ThemeHandlerInterface $theme_handler) {
    $plugin_interface = 'Drupal\widgets\Plugin\Widget\WidgetInterface';
    $plugin_definition_annotation_name = 'Drupal\widgets\Annotation\Widget';
    parent::__construct("Plugin/widget", $namespaces, $module_handler, $plugin_interface, $plugin_definition_annotation_name);
    $this->discovery = new YamlDiscoveryDecorator($this->getDiscovery(), 'widgets', $module_handler->getModuleDirectories() + $theme_handler->getThemeDirectories());
    $this->themeHandler = $theme_handler;

    $this->defaults += [
      'bar' => 'foo',
      'class' => 'Drupal\widgets\Plugin\Widget\WidgetDefault',
    ];

    $this->setCacheBackend($cache_backend, 'widgets');
    $this->alterInfo('widgets');
  }
  ...
  public function alterThemeImplementations(array &$theme_registry) {
    ...
    foreach ($theme_registry as $theme_hook => &$info) {
      if (in_array($theme_hook, $layout_theme_hooks) || (!empty($info['base hook']) && in_array($info['base hook'], $layout_theme_hooks))) {
        ...
        array_splice($info['preprocess functions'], $index, 0, '_widget_plugin_preprocess_widget');
      }
    }
  }
